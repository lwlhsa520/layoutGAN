...
def subprocess_fn(rank, args, temp_dir):
    ...
    if rank == 0 and args.verbose:
        ...
        bbox = torch.empty([1, G.bbox_dim, 5], device=device)
        misc.print_module_summary(G, [z, bbox])
    ...
    for metric in args.metrics:
        ...
        result_dict = metric_main.calc_metric(metric=metric, G=G, dataset_kwargs=args.dataset_kwargs, dataset1_kwargs=args.dataset1_kwargs,
...
def calc_metrics(ctx, network_pkl, metrics, data, data1, mirror, gpus, verbose):
    """Calculate quality metrics for previous training run or pretrained network pickle.
    ...
    if data is not None:
        args.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset1.ImageFolderDataset', path=data, bbox_dim=256)
    elif network_dict['training_set_kwargs'] is not None:
        args.dataset_kwargs = dnnlib.EasyDict(network_dict['training_set_kwargs'])
    else:
        ctx.fail('Could not look up dataset options; please specify --data')

    args.dataset1_kwargs = dnnlib.EasyDict(class_name='training.dataset1.ImageFolderDataset', path=data1, bbox_dim=256)
