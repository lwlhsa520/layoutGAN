...
class RenderNet(nn.Module):
    def __init__(self,
        ...
        for res in self.block_resolutions:
            ...
            block = SynthesisBlock(in_channels, out_channels, w_dim=w_dim, resolution=res,
                                   img_channels=img_channels,mid_size=mid_size, mask_channels=self.mask_channels, is_last=is_last, use_fp16=use_fp16, **block_kwargs)
            ...
            #     fus = Conv2dLayer(self.mask_channels, out_channels, kernel_size=3)
            #     setattr(self, f'fus{res}', fus)


    def forward(self, x, ws, get_feats = False, **block_kwargs):
        block_ws = []
        with torch.autograd.profiler.record_function('split_ws'):
            misc.assert_shape(ws, [None, self.num_ws, self.w_dim])
            ws = ws.to(torch.float32)
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            ...
            block = getattr(self, f'b{res}')
            x, img, mask_x = block(x, img, mask_x, cur_ws, **block_kwargs)

            if get_feats and res in self.get_feat_res:
                feats[res] = x
...
class SimGenerator(nn.Module):
    ...
    def forward(self, z, bbox, truncation_psi=1, truncation_cutoff=None, isTrain=True, get_feat=False):
        ...
        if isTrain:
            ...
            return img, sem_mask, mid_masks*2 - 1, feats
...
def color_mask(mask):
    ...
    weight = torch.arange(ch, 0, -1).to(mask.device).unsqueeze(0).repeat([B, 1]).view(B, ch, 1, 1)
    mask = mask * weight

    num = len(color_map) - 1
    sample_seg = torch.argmax(mask, dim=1).detach().cpu().numpy()
    new_mask = np.zeros((mask.shape[0], mask.shape[1], mask.shape[2], 3), dtype=np.uint8)
    new_mask[sample_seg == 0] = color_map[num]
    for key in range(ch-1):
        new_mask[sample_seg==key] = color_map[key%num]
    return new_mask.transpose(0, 3, 1, 2) # BCHW
...
class DualDiscriminator(torch.nn.Module):
    def __init__(self,
        ...
        self.ob32 = DiscriminatorBlock(channels_dict[32], channels_dict[32], channels_dict[16], resolution=32, img_channels=img_channels, down=False,
        first_layer_idx=cur_layer_idx, use_fp16=(32 >= fp16_resolution), **block_kwargs, **common_kwargs)
        cur_layer_idx += self.ob32.num_layers

        self.ob16 = DiscriminatorBlock(channels_dict[32], channels_dict[32], channels_dict[16], resolution=16, img_channels=img_channels, down=False,
                                       first_layer_idx=cur_layer_idx, use_fp16=(16 >= fp16_resolution), **block_kwargs, **common_kwargs)
        cur_layer_idx += self.ob16.num_layers

        self.ob_last = DiscriminatorBlock(channels_dict[8], channels_dict[8],  channels_dict[4],  first_layer_idx=cur_layer_idx, use_fp16 = (8 >= fp16_resolution),  resolution=8, img_channels=img_channels, **block_kwargs)
        ...
        self.ob4 = DiscriminatorEpilogue(channels_dict[4], resolution=4, getVec=False, **epilogue_kwargs, **common_kwargs)
