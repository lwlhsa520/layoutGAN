...
from training.blocks import SEBlock
...
class RenderNet(nn.Module):
    def __init__(self,
        ...
        for res in self.block_resolutions:
            ...
            if res not in [self.in_resolution, self.img_resolution]:
                se = SEBlock(self.mid_channels, channels_dict[res])
                setattr(self, f'se_{res}', se)
...
class DualDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, bbox, mask, condition=None, **block_kwargs):
        x = None
        y = None
