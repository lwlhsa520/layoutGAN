...
from torch_utils.common import Perceptual_loss134
from torch_utils.custom_ops import color_map
...
def color_mask(mask):
    ...
    B, ch = mask.shape[:2]
    assert ch<=256,'mask channels must be <= 256'
    mask = torch.cat([mask, torch.ones([B, 1, mask.shape[2], mask.shape[3]], dtype=mask.dtype, device=mask.device)*0.1], dim=1)
    ch = ch+1
    weight = torch.arange(1, ch+1).flip(0).to(mask.device).unsqueeze(0).repeat([B, 1]).view(B, ch, 1, 1)
    mask = mask * weight

    num = len(color_map)
    sample_seg = torch.argmax(mask, dim=1).detach().cpu().numpy()
    new_mask = np.zeros((mask.shape[0], mask.shape[2], mask.shape[3], 3), dtype=np.uint8)
    for key in range(0, ch-1):
        new_mask[sample_seg==key] = color_map[key % num]
    return new_mask # BHWC
...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    # Load data.
    image, mask, bbox = zip(*[training_set[i] for i in grid_indices])
    return (gw, gh), np.stack(image), np.stack(mask), np.stack(bbox)
...
def training_loop(
    ...
    test_set = dnnlib.util.construct_class_by_name(**test_set_kwargs)  # subclass of training.dataset.Datase
    ...
    lpips = Perceptual_loss134().eval().to(device)
    ...
    if rank == 0:
        ...
        c = torch.empty([batch_gpu, 0], device=device)
        bbox = torch.from_numpy(training_set[0][2]).unsqueeze(0).repeat(batch_gpu, 1, 1).to(device)
        img, mask, _, _ = misc.print_module_summary(G, [z, bbox])
        misc.print_module_summary(D, [img, bbox, mask])

    # Distribute across GPUs.
    if rank == 0:
        print(f'Distributing across {num_gpus} GPUs...')
    for module in [G, D, G_ema, lpips]:
    ...
    loss = dnnlib.util.construct_class_by_name(device=device, G=G, G_ema=G_ema, D=D, LPIPS=lpips, **loss_kwargs) # subclass of training.loss.Loss
    phases = []
    ...
    if rank == 0:
        ...
        grid_size, images, masks, bbox = setup_snapshot_image_grid(training_set=training_set)
        save_image_grid(images, os.path.join(run_dir, 'reals.png'), drange=[0,255], grid_size=grid_size)
        # grid_c = torch.from_numpy(labels).to(device).split(batch_gpu)
        # grid_c = torch.from_numpy(labels).to(device).split(batch_gpu)
        gen_imgs, gen_masks, gen_mid_masks = [], [], []
        gen_masks2 = []
        grid_size, images, masks, bbox = setup_snapshot_image_grid(training_set=test_set)
        grid_z = torch.randn([images.shape[0], G.z_dim], device=device).split(batch_gpu)
        grid_bbox = torch.from_numpy(bbox).to(device).split(batch_gpu)
        for z, box in zip(grid_z, grid_bbox):
            gen_img, gen_mask, _, gen_mid_mask = G_ema(z=z, bbox=box)
            gen_imgs.append(gen_img)
            gen_masks.append(gen_mask)
            gen_mid_masks.append(F.interpolate(gen_mid_mask, gen_mask.shape[2:4], mode='nearest'))
        # image = torch.cat([G_ema(z=z, bmask=bbox_mask(device, box, 64, 64), noise_mode='const').cpu() for z, box in zip(grid_z, grid_bbox)]).numpy()
        save_image_grid(torch.cat(gen_imgs).cpu().numpy(), os.path.join(run_dir, f'fakes_image_init.png'),
                        drange=[-1, 1], grid_size=grid_size)
        save_image_grid(torch.cat(gen_masks).cpu().numpy(), os.path.join(run_dir, f'fakes_mask_init.png'),
                        drange=[-1, 1], grid_size=grid_size)
        save_image_grid(torch.cat(gen_mid_masks).cpu().numpy(), os.path.join(run_dir, f'fakes_mid_mask_init.png'),
                        drange=[-1, 1], grid_size=grid_size)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            phase_real_img, phase_real_mask, phase_real_bbox = next(training_set_iterator)
            phase_real_img = (phase_real_img.to(device).to(torch.float32) / 127.5 - 1).split(batch_gpu) # adjust[-1, 1]
            phase_real_mask = (phase_real_mask.to(device).to(torch.float32) / 127.5 - 1).split(batch_gpu) # adjust[0, 1]
            phase_real_bbox = phase_real_bbox.to(device).split(batch_gpu)
        ...
        for phase, phase_gen_z in zip(phases, all_gen_z):
            ...
            for real_img, real_mask, real_bbox, gen_z in zip(phase_real_img, phase_real_mask, phase_real_bbox, phase_gen_z):
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_mask=real_mask, real_bbox=real_bbox, gen_z=gen_z, gain=phase.interval, cur_nimg=cur_nimg)
            phase.module.requires_grad_(False)

            # for name, param in phase.module.named_parameters():
            #     if param.grad is None:
            #         print(phase.name, name)
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            g_imgs, g_masks, g_m_masks = torch.cat(gen_imgs).cpu().numpy(), torch.cat(gen_masks).cpu().numpy(), torch.cat(gen_mid_masks).cpu().numpy()
            # images = torch.cat([G_ema(z=z, noise_mode='const').cpu() for z in grid_z]).numpy()
            # image = torch.cat([G_ema(z=z, bmask=bbox_mask(device, box, 64, 64), noise_mode='const').cpu() for z, box in zip(grid_z, grid_bbox)]).numpy()
            save_image_grid(g_imgs, os.path.join(run_dir, f'fakes_image{cur_nimg // 1000:06d}.png'), drange=[-1, 1],
                            grid_size=grid_size)
            save_image_grid(g_masks, os.path.join(run_dir, f'fakes_mask{cur_nimg // 1000:06d}.png'), drange=[-1, 1],
                            grid_size=grid_size)
            save_image_grid(g_m_masks, os.path.join(run_dir, f'fakes_mid_mask{cur_nimg // 1000:06d}.png'),
                            drange=[-1, 1], grid_size=grid_size)
        ...
        if cur_tick and (snapshot_data is not None) and (len(metrics) > 0):
            ...
            for metric in metrics:
                ...
                result_dict = metric_main.calc_metric(metric=metric, G=snapshot_data['G_ema'],
                    dataset_kwargs=training_set_kwargs, dataset1_kwargs=test_set_kwargs, num_gpus=num_gpus,
